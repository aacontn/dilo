//! Portable mode support for Dilo.
//!
...
pub fn init() {
    PORTABLE_DATA_DIR.get_or_init(|| {
        ...
        let is_portable = if is_valid_portable_marker(&marker_path) {
            ...
            let _ = std::fs::write(&marker_path, "Dilo Portable Mode");
            true
        } else {
            false
        };
...
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| {
            let s = s.trim();
            // "Handy Portable Mode" is accepted for back-compat: portable
            // installs set up with v0.1.0–0.1.4 wrote the old magic string.
            s.starts_with("Dilo Portable Mode") || s.starts_with("Handy Portable Mode")
        })
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        ...
        write!(f, "Dilo Portable Mode").unwrap();
