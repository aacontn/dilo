...
//! `open()`, `start()`, `stop() -> Vec<f32>`) para que el código que la
//! consuma después no tenga que aprender un segundo modelo. Se aparta en un
//! solo punto: no hay parámetro de dispositivo en `open()` — un tap global no
//! selecciona un dispositivo de entrada, captura todo lo que suena en el
//! equipo.
//!
...
/// el tap stereo que arma `open()` no debería producir este layout, pero si
...
pub fn mix_planar_to_mono(channels: &[&[f32]]) -> Vec<f32> {
    match channels.len() {
        0 => Vec::new(),
        1 => channels[0].to_vec(),
        _ => {
...
mod tests {
    ...
    fn mezcla_estereo_intercalado_a_mono() {
        ...
        let out = mix_interleaved_to_mono(&[1.0, 3.0, 2.0, 4.0], STEREO_CHANNELS);
        assert_eq!(out, vec![2.0, 3.0]);
    }

    #[test]
    fn mono_intercalado_es_passthrough() {
        let out = mix_interleaved_to_mono(&[0.1, -0.2, 0.3], 1);
        assert_eq!(out, vec![0.1, -0.2, 0.3]);
    }

    #[test]
    fn canales_en_cero_no_divide_por_cero() {
        assert_eq!(mix_interleaved_to_mono(&[1.0, 2.0], 0), Vec::<f32>::new());
    }

    #[test]
    fn intercalado_descarta_frame_incompleto() {
        // 5 muestras / 2 canales = 2 frames completos; el 5to elemento sobra.
        let out = mix_interleaved_to_mono(&[1.0, 1.0, 2.0, 2.0, 99.0], 2);
    ...
    fn mezcla_planar_dos_canales() {
        ...
        let out = mix_planar_to_mono(&[&left, &right]);
    ...
    fn planar_un_canal_es_passthrough() {
        let ch = [0.5f32, -0.5];
        let out = mix_planar_to_mono(&[&ch]);
        assert_eq!(out, vec![0.5, -0.5]);
    }

    #[test]
    fn planar_sin_canales_da_vacio() {
        let out = mix_planar_to_mono(&[]);
        assert!(out.is_empty());
    }
