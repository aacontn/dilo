...
const pcm = Buffer.concat([pcmOf("s1.wav"), gap, pcmOf("s2.wav"), gap, pcmOf("s3.wav")]);
...
async function probe(label: string, config: Record<string, unknown> | null) {
  ...
  const body: any = {
    ...
    input: [{ type: "audio", mime_type: "audio/wav", data: wav.toString("base64") }],
  };
  ...
  if (!res.ok || !json) { console.log(JSON.stringify(json)?.slice(0, 400)); return; }
...
await probe("verbatim + diarization + word_timestamps", { diarization: true, word_timestamps: true });
