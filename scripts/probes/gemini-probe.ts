...
async function probeLive(mode: "SMART" | "VERBATIM") {
  ...
  const stamp = (s: string) =>
    events.push(`t+${((now() - t0) / 1000).toFixed(2)}s  ${s}`);
  ...
  ws.onmessage = async (ev: MessageEvent) => {
    ...
    try {
      ...
      stamp(`frame no-JSON (${raw.length}b)`);
      return;
    }
    ...
    if (msg.error) {
      stamp(`ERROR servidor: ${JSON.stringify(msg.error).slice(0, 300)}`);
      return;
    }
    ...
    if (interim?.text) {
      if (firstPartialAt === null) {
        firstPartialAt = now() - t0;
        stamp(`PRIMER PARCIAL: "${interim.text.slice(0, 60)}"`);
      }
    ...
    if (content?.goAway !== undefined || msg.goAway !== undefined) {
      stamp("goAway");
      return;
    }
...
async function probeBatch(mime: string, body: Buffer, label: string) {
  ...
  const res = await fetch(`${ENDPOINT}/v1beta/interactions`, {
    ...
    body: JSON.stringify({
      ...
      input: [
        { type: "audio", mime_type: mime, data: body.toString("base64") },
      ],
  ...
  if (!res.ok || !json) {
    console.log(
      `HTTP ${res.status} en ${secs}s — ${JSON.stringify(json)?.slice(0, 300)}`,
    );
    return;
  }
