...
un tope duro de 8s por turno (cabe en la ventana de 10s del modelo de segmentación, su caso más preciso) más un corte por voz _dentro_ de cada trozo (`split_turn_into_pieces` en `managers/meeting.rs`), que separa por cambio de hablante, funde solapes menores y descarta como mezclada la voz que se pisa más del 60%. El streaming de Nemotron sigue siendo la mejora de percepción pendiente; esto es la mejora de corrección que la precede.
